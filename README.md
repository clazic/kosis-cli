...
Claude Code의 채팅창에서 (macOS / Windows / Linux 공통):
...
**Windows 참고:**
- 바이너리 자동 다운로드 시 `scripts\install-binary.ps1` 이 실행됩니다.
- PowerShell 실행 정책 오류 시: `Set-ExecutionPolicy RemoteSigned -Scope CurrentUser`
- 한글 깨짐 시: `chcp 65001` 실행 후 터미널 재시작

---

### 방법 2: npm (macOS / Windows / Linux)
...
- 바이너리: `%USERPROFILE%\.claude\skills\kosis\apps\kosis.exe`
- `kosis` 명령 실행: npm이 설치한 `bin\kosis` shim 사용 (별도 PATH 불필요)

**Windows 참고:**
- `tar` 명령이 필요합니다 (Windows 10 1803 이상 기본 내장)
- 이전 Windows 버전은 [방법 4](#방법-4-windows-powershell)를 사용하세요.
...
| `tar` 명령 없음 | Windows 10 1803 미만 — [GitHub Releases](https://github.com/clazic/kosis-cli/releases)에서 직접 다운로드 |
...
| OS | 아키텍처 | 설치 방법 |
|----|---------|---------|
| macOS (Apple Silicon) | arm64 | 방법 1, 2, 3 |
| macOS (Intel) | amd64 | 방법 1, 2, 3 |
| Linux | amd64, arm64 | 방법 1, 2, 3 |
| Windows 10 1803+ | amd64 | 방법 1, 2, 4 |
| Windows (구버전) | amd64 | 방법 4 또는 직접 다운로드 |
...
- 릴리스: [GitHub Releases](https://github.com/clazic/kosis-cli/releases)
